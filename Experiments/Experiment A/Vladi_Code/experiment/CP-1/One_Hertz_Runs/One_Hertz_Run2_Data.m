...
Run_OneHz=load('CP1_1Hz_r2','-mat')

%Convert the -mat file into a table. Convert the table into an array. 
Run_OneHz_Data=table2array(struct2table(Run_OneHz));

%The following array 300 entries of time. 
Time=Run_OneHz_Data(:,1);

%The next three arrays store the Gyro x,y, and z data for an IMU.  
IMU_1_GyroX=Run_OneHz_Data(:,2);
IMU_1_GyroY=Run_OneHz_Data(:,3);
IMU_1_GyroZ=Run_OneHz_Data(:,4);

%The next three arrays store the acceleration x,y, and z data for an IMU.  
IMU_1_AccX=Run_OneHz_Data(:,5);
IMU_1_AccY=Run_OneHz_Data(:,6);
IMU_1_AccZ=Run_OneHz_Data(:,7);

%The next three arrays store the gyro x,y, and z data for an IMU.  
IMU_2_GyroX=Run_OneHz_Data(:,8);
IMU_2_GyroY=Run_OneHz_Data(:,9);
IMU_2_GyroZ=Run_OneHz_Data(:,10);

%The next three arrays store the acceleration x,y, and z data for an IMU.
IMU_2_AccX=Run_OneHz_Data(:,11);
IMU_2_AccY=Run_OneHz_Data(:,12);
IMU_2_AccZ=Run_OneHz_Data(:,13);

%The following array stores the position of the motor through the trial. 
Motor_position=Run_OneHz_Data(:,14);
